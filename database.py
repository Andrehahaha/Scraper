...
def _parse_prezzo(p: str) -> float:
    if not p:
        return None
    try:
        return float(p.replace("€", "").replace(",", ".").replace("\xa0", "").strip())
    except Exception:
        return None
    ...
    with get_conn() as conn:
        ...
        for p in prodotti:
            ...
            if nome in vecchi_dict:
                prezzo_vecchio, _ = vecchi_dict[nome]
                ...
                if p_new is not None and p_old is not None and p_new != p_old:
                    variazione = "diminuito" if p_new < p_old else "aumentato"
                    conn.execute("""
        ...
        conn.execute("DELETE FROM prodotti WHERE negozio=? AND categoria=?", (negozio, categoria))
        conn.executemany("""
...
def carica_categoria(negozio: str, categoria: str) -> list:
    ...
    return [{"nome": r[0], "prezzo": r[1], "prezzo_originale": r[2], "sconto": r[3],
             "sconto_percentuale": r[4], "immagine": r[5], "link": r[6], "aggiornato_il": r[7]}
            for r in rows]
        ...
        if negozio:
            rows = conn.execute("""
                SELECT negozio, categoria, nome, prezzo, prezzo_originale,
                       sconto, sconto_percentuale, immagine, link, aggiornato_il
                FROM prodotti WHERE sconto_percentuale >= ? AND negozio = ?
                ORDER BY sconto_percentuale DESC, nome ASC
            ...
            rows = conn.execute("""
                SELECT negozio, categoria, nome, prezzo, prezzo_originale,
                       sconto, sconto_percentuale, immagine, link, aggiornato_il
                FROM prodotti WHERE sconto_percentuale >= ?
                ORDER BY sconto_percentuale DESC, nome ASC
            """, (soglia,)).fetchall()
    ...
    with get_conn() as conn:
        if negozio:
            rows = conn.execute("""
                ...
                FROM storico_prezzi WHERE negozio=?
            ...
            rows = conn.execute("""
                SELECT negozio, nome, prezzo_corrente, prezzo_originale,
                       prezzo_vecchio, variazione, data
                FROM storico_prezzi ORDER BY data DESC LIMIT ?
            """, (limite,)).fetchall()
    return [{"negozio": r[0], "nome": r[1], "prezzo_corrente": r[2],
             "prezzo_originale": r[3], "prezzo_vecchio": r[4],
             "variazione": r[5], "data": r[6]} for r in rows]


def variazioni_per_marca(negozio: str, giorni: int = 7) -> list:
    with get_conn() as conn:
        rows = conn.execute("""
            SELECT nome, prezzo_corrente, prezzo_originale, prezzo_vecchio, variazione, data
            FROM storico_prezzi WHERE negozio=? ORDER BY data DESC
        """, (negozio,)).fetchall()
    return [{"nome": r[0], "prezzo_corrente": r[1], "prezzo_originale": r[2],
             "prezzo_vecchio": r[3], "variazione": r[4], "data": r[5]} for r in rows]


def storico_prezzi_prodotto(neg: str, nome: str, limite: int = 50) -> list:
    with get_conn() as conn:
        rows = conn.execute("""
            SELECT prezzo_corrente, prezzo_originale, prezzo_vecchio, variazione, data
            FROM storico_prezzi WHERE negozio=? AND nome=?
            ORDER BY data DESC LIMIT ?
        """, (neg, nome, limite)).fetchall()
    return [{"prezzo_corrente": r[0], "prezzo_originale": r[1],
             "prezzo_vecchio": r[2], "variazione": r[3], "data": r[4]} for r in rows]


# ── FLASH SALE ────────────────────────────────────────────────
def flash_sale(soglia_calo: int = 20, ore: int = 24) -> list:
    """Prodotti il cui prezzo è calato di almeno soglia_calo% nell'ultima sessione."""
    with get_conn() as conn:
        rows = conn.execute("""
            SELECT s.negozio, s.nome, s.prezzo_corrente, s.prezzo_vecchio,
                   s.variazione, s.data, p.categoria, p.immagine, p.link, p.sconto_percentuale
    ...
    for r in rows:
        try:
            p_new = _parse_prezzo(r[2])
            p_old = _parse_prezzo(r[3])
            if p_new and p_old and p_old > 0:
                calo = (p_old - p_new) / p_old * 100
                if calo >= soglia_calo:
                    risultati.append({
                        "negozio": r[0], "nome": r[1],
                        "prezzo_corrente": r[2], "prezzo_vecchio": r[3],
                        "calo_percentuale": round(calo),
                        "data": r[4], "categoria": r[6],
                        "immagine": r[7], "link": r[8],
                    })
        except Exception:
            continue
    return risultati
...
# ── WISHLIST ──────────────────────────────────────────────────
def aggiungi_wishlist(negozio: str, categoria: str, nome: str,
                      prezzo_target: str = "", prezzo_attuale: str = "",
                      immagine: str = "", link: str = ""):
    ora = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    with get_conn() as conn:
        conn.execute("""
            INSERT OR REPLACE INTO wishlist
                (negozio, categoria, nome, prezzo_target, prezzo_attuale, immagine, link, aggiunto_il)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (negozio, categoria, nome, prezzo_target, prezzo_attuale, immagine, link, ora))
        conn.commit()
...
def carica_wishlist() -> list:
    with get_conn() as conn:
        rows = conn.execute("""
            SELECT w.negozio, w.categoria, w.nome, w.prezzo_target,
                   w.prezzo_attuale, w.immagine, w.link, w.aggiunto_il,
                   p.prezzo, p.sconto, p.sconto_percentuale
            FROM wishlist w
    ...
    risultati = []
    for r in rows:
        prezzo_target = _parse_prezzo(r[3])
        prezzo_attuale = _parse_prezzo(r[8] or r[4])
        target_raggiunto = (prezzo_target is not None and
                            prezzo_attuale is not None and
                            prezzo_attuale <= prezzo_target)
        risultati.append({
            "negozio": r[0], "categoria": r[1], "nome": r[2],
            "prezzo_target": r[3], "prezzo_attuale": r[4],
            "immagine": r[8+2] or r[5], "link": r[6], "aggiunto_il": r[7],
            "prezzo": r[8] or r[4], "sconto": r[9] or "",
            "sconto_percentuale": r[10] or 0,
...
# ── ALERT ─────────────────────────────────────────────────────
def alert_gia_inviato(negozio: str, nome: str, tipo: str, ore: int = 23) -> bool:
    with get_conn() as conn:
        row = conn.execute("""
            SELECT inviato_il FROM alert_inviati
            WHERE negozio=? AND nome=? AND tipo=?
            ORDER BY inviato_il DESC LIMIT 1
        """, (negozio, nome, tipo)).fetchone()
    if not row:
        return False
    try:
...
def registra_alert(negozio: str, nome: str, tipo: str):
    ora = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    with get_conn() as conn:
        conn.execute("""
            INSERT INTO alert_inviati (negozio, nome, tipo, inviato_il)
            VALUES (?, ?, ?, ?)
        """, (negozio, nome, tipo, ora))
        conn.commit()
